...
def get_state_splitter_joiner(profile_names, scalar_names, lookback, lookahead, profile_length):
    num_profiles = len(profile_names)
    num_scalars = len(scalar_names)
    state_dim = num_profiles*profile_length + num_scalars*(lookback+1)
    ...
    scalar_inputs = [Input((lookahead, lookback+1), name='input_' + nm)
                     for nm in scalar_names]
    scalars = Concatenate(
        axis=-1)(scalar_inputs) if num_scalars > 1 else scalar_inputs[0]
    ...
    profile_outputs = [Reshape((1, profile_length))(
        Cropping1D((i*profile_length, state_dim-(i+1)*profile_length),
                   name='output_' + nm)(y)) for i, nm in enumerate(profile_names)]
    scalar_outputs = [Reshape((1, lookback+1))(
        Cropping1D((num_profiles*profile_length + i*(lookback+1),
                    state_dim-num_profiles*profile_length - (i+1)*(lookback+1)),
...
def get_control_encoder_dense(actuator_names, control_latent_dim,
                              std_activation, num_layers, **kwargs):
...
def get_state_encoder_dense(profile_names, scalar_names, lookback, profile_length,
                            state_latent_dim, std_activation, num_layers, **kwargs):
    layer_scale = kwargs.get('layer_scale', 1)
    num_profiles = len(profile_names)
    num_scalars = len(scalar_names)
    state_dim = num_profiles*profile_length + num_scalars*(lookback+1)
    ...
    joiner, _ = get_state_splitter_joiner(
        profile_names, scalar_names, lookback, 1, profile_length)
...
def get_state_decoder_dense(profile_names, scalar_names, lookback, profile_length,
                            state_latent_dim, std_activation, num_layers, **kwargs):
    layer_scale = kwargs.get('layer_scale', 1)
    num_profiles = len(profile_names)
    num_scalars = len(scalar_names)
...
def get_latent_linear_model(state_latent_dim, control_latent_dim, lookback,
                            lookahead, regularization=None):
    if regularization is None:
        regularization = {'l1A': 0,
                          'l2A': 0,
                          'l1B': 0,
                          'l2B': 0}
    xi = Input((lookahead+1, state_latent_dim))
    ui = Input((lookback+lookahead, control_latent_dim))
    x0 = Cropping1D((0, lookahead))(xi)
    x1 = Cropping1D((1, 0))(xi)
    u = Cropping1D((lookback, 0))(ui)
    ...
    x1est = Reshape((lookahead, state_latent_dim))(AB(u, x0))
...
def make_autoencoder(state_encoder, state_decoder, control_encoder, control_decoder,
                     state_joiner, state_splitter, control_joiner, control_splitter,
                     profile_names, scalar_names, actuator_names,
                     state_latent_dim, control_latent_dim, lookback, lookahead, regularization=None):
    num_profiles = len(profile_names)
    num_scalars = len(scalar_names)
    num_actuators = len(actuator_names)
    state_dim = num_profiles*profile_length + num_scalars*(lookback+1)
    linear_model = get_latent_linear_model(state_latent_dim, control_latent_dim,
                                           lookback, lookahead, regularization=regularization)
