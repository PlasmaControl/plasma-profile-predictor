...
def get_state_decoder_batched(profile_names, scalar_names, profile_length,
                            state_latent_dim, std_activation, batch_size=None, **kwargs):
    layer_scale = kwargs.get('layer_scale', 1)
    num_layers = kwargs.get('num_layers', 6)
    num_profiles = len(profile_names)
    num_scalars = len(scalar_names)
    state_dim = num_profiles*profile_length + num_scalars
    x_input = layers.Input(batch_shape=(batch_size, state_latent_dim*2))
    x_input_resh = layers.Reshape((state_latent_dim*2, 1))(x_input)    
    x_1 = layers.Reshape((state_latent_dim,))(layers.Cropping1D((0,state_latent_dim))(x_input_resh))
    x_2 = layers.Reshape((state_latent_dim,))(layers.Cropping1D((state_latent_dim,0))(x_input_resh))
    for i in range(num_layers-1,0,-1):
        units = int(state_latent_dim + (state_dim-state_latent_dim)
                    * ((num_layers-i-1)/(num_layers-1))**layer_scale)
        shared = layers.Dense(units = units, activation=std_activation,use_bias=True)
        x_1 = shared(x_1)
        x_2 = shared(x_2)
    linear_shared = layers.Dense(units=state_dim, activation='linear')
    output1 = layers.Reshape((state_dim,))(linear_shared(x_1))
    output2 = layers.Reshape((state_dim,))(linear_shared(x_2))
    outputs = []
    outputs.append(output1)
    outputs.append(output2)
    decoder = Model(inputs = x_input, outputs = outputs, name = 'test')
    return decoder
...
def make_LRAN(state_encoder_type, state_decoder_type, control_encoder_type, control_decoder_type, state_encoder_kwargs, state_decoder_kwargs, control_encoder_kwargs, control_decoder_kwargs, profile_names, scalar_names, actuator_names, state_latent_dim, control_latent_dim, profile_length, lookback, lookahead, batch_size=None, **kwargs):
    ...
    state_decoder_test = get_state_decoder_batched(profile_names,
                                                       scalar_names,
                                                       profile_length,
                                                       state_latent_dim,
                                                       batch_size=batch_size,
    ...
    latent_future_predict_dec = layers.Concatenate(axis=1)([layers.Cropping1D((0,lookahead))(x), latent_future_predict])
    
    # This *should* use only one instance of the decoder model (although the plot might say otherwise)
    input_concat = layers.Concatenate(axis=-1)([latent_future_predict_dec,x])
    outputs=[]
    for out in state_decoder_test.outputs:
        outputs.append(layers.TimeDistributed(Model(state_decoder_test.inputs, out))(input_concat))
    x_out_future, x_out_coords = outputs
   
    # Residuals
    latent_residual = layers.subtract([latent_future, latent_future_predict],
                                      name='linear_system_residual')
    x_res = layers.subtract([x_input, x_out_future], name='x_residual')
    u_res = layers.subtract([u_out, u_input], name='u_residual')
    coord_res = layers.subtract([x_out_coords, x_input], name='coord_residual')
