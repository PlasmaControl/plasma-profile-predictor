   ...
   """
    ...
    for i in range(num_profiles):
        profile_inputs.append(
            keras.layers.Input(profile_inshape, name="input_" + input_profile_names[i])
        )
    ...
    for i in range(num_actuators):
        ...
        actuator_future_inputs.append(
            layers.Input(
                future_actuator_inshape,
                name="input_future_{}".format(actuator_names[i]),
            )
        )
        future_actuators.append(
            layers.Reshape((lookahead, 1))(actuator_future_inputs[i])
        )

        past_scalar_inputs.append(
            layers.Input(
                past_scalar_inshape, name="input_past_{}".format(actuator_names[i])
            )
        )
        previous_scalars.append(
            # Reshape((lookbacks[actuator_names[i]], 1))(actuator_past_inputs[i]))
            layers.Reshape(
                (max(max_scalar_lookback + 1, max_actuator_lookback + 1), 1)
            )(past_scalar_inputs[i])
        )

    for i in range(num_scalars):
        past_scalar_inputs.append(
            layers.Input(
                past_scalar_inshape, name="input_{}".format(scalar_input_names[i])
            )
        )
        previous_scalars.append(
            # Reshape((lookbacks[actuator_names[i]], 1))(actuator_past_inputs[i]))
    ...
    actuator_effect = rnn_layer(profile_length, activation=std_activation)(
        previous_scalars
    )
    actuator_effect = layers.Reshape(target_shape=(profile_length, 1))(actuator_effect)

    future_actuator_effect = rnn_layer(profile_length, activation=std_activation)(
        future_actuators
    )
    future_actuator_effect = layers.Reshape(target_shape=(profile_length, 1))(
        future_actuator_effect
    )
    ...
    for i in range(num_targets):
        ...
        current_profiles_processed_1 = layers.Conv1D(
            filters=max_channels, kernel_size=2, padding="same", activation="relu"
        )(current_profiles_processed_0)
        current_profiles_processed_2 = layers.Conv1D(
            filters=max_channels, kernel_size=4, padding="same", activation="relu"
        )(current_profiles_processed_1)
        current_profiles_processed_3 = layers.Conv1D(
            filters=max_channels, kernel_size=8, padding="same", activation="relu"
    ...
    model = Model(
        inputs=profile_inputs + past_scalar_inputs + actuator_future_inputs,
        outputs=prof_act,
    )
...
def build_dumb_simple_model(
    ...
    **kwargs
    ...
    for i in range(num_profiles):
        ...
        # size 65,
        profiles.append(
            Dense(units=profile_length, activation=std_activation)(profile_inputs[i])
        )
    ...
    if num_scalars > 0:
        ...
        scalars = []
        for i in range(num_scalars):
            scalar_inputs.append(Input((1,), name="input_" + scalar_input_names[i]))
            # size 1,
            scalars.append(
                Dense(units=profile_length, activation=std_activation)(scalar_inputs[i])
            )
            # size 65,
    ...
    for i in range(num_actuators):
        ...
        # size lookahead, 1
        actuators[i] = Dense(units=profile_length, activation=std_activation)(
            actuators[i]
        )
        # size lookahead, 65
    ...
    for i in range(num_targets):
        ...
        for j in range(len(outputs[i])):
            outputs[i][j] = Dense(units=profile_length, activation=std_activation)(
                outputs[i][j]
            )
            # size 65,
        ...
        if kwargs.get("predict_mean"):
            outputs[i] = Dense(units=profile_length, activation=std_activation)(
                outputs[i]
            )
