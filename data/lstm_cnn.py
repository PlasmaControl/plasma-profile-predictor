...
def get_datasets(batch_size, input_filename, output_dirname, preprocess,
    ...
    if (preprocess):
        data_package = preprocess_data(input_filename, output_dirname, sigs_0d,
                                       sigs_1d, sigs_predict, n_components,
                                       avg_window, lookback, delay, train_frac,
                                       val_frac, False)
...
def preprocess_data(input_filename, output_dirname, sigs_0d, sigs_1d, sigs_predict,
    ...
    data_all_times['shot'] = np.array(
        [[shot]*data[shot][sigs[0]].shape[0] for shot in all_shots])
    ...
    for sig in sigs:
        data_all_times_normed[sig] = normalize(
            data_all_times[sig], means[sig], stds[sig])
    ...
    for subset in subsets:
        ...
        for sig in sigs_predict:
            final_target[sig] = data_all_times_normed[sig][indices[subset] +
        ...
        for sig in sigs_0d+sigs_1d:
            final_input[sig] = {}
            final_input[sig] = np.stack([data_all_times_normed[sig][indices[subset]+offset]
                                         for offset in range(-lookback, delay+1)], axis=1)
        final_input_0d = np.concatenate(
            [final_input[sig][:, :, np.newaxis] for sig in sigs_0d], axis=2)
        final_input_1d = np.concatenate(
            [final_input[sig] for sig in sigs_1d], axis=2)
        final_input_1d[:, -delay:, :] = pad_1d_to
        input_data[subset] = np.concatenate(
            [final_input_0d, final_input_1d], axis=2)

    if save_data:
        for subset in subsets:
            save_obj(data_all_times['time'][indices[subset]], os.path.join(
                output_dirname, '{}_time'.format(subset)))
            save_obj(data_all_times['shot'][indices[subset]], os.path.join(
                output_dirname, '{}_shot'.format(subset)))
            save_obj(target[subset], os.path.join(
                output_dirname, '{}_target'.format(subset)))
            save_obj(input_data[subset], os.path.join(
                output_dirname, '{}_data'.format(subset)))
