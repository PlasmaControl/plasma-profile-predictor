...
def process_results_folder(dir_path):
    ...
    all_files = os.listdir(os.path.abspath(dir_path))
    model_files = [
        os.path.abspath(dir_path) + "/" + f for f in all_files if f.endswith(".h5")
    ]
...
def write_autoencoder_results(model, scenario):
    ...
    if "image_path" not in scenario.keys():
        scenario["image_path"] = (
            "https://jabbate7.github.io/plasma-profile-predictor/results/"
            + scenario["runname"]
        )
    ...
    try:
        creds = ServiceAccountCredentials.from_json_keyfile_name(
            os.path.expanduser("~/plasma-profile-predictor/drive-credentials.json"),
            scope,
        )
    ...
    results_dir = os.path.expanduser(
        "~/plasma-profile-predictor/results/" + scenario["runname"]
    )
    ...
    if not os.path.exists("training.png"):
        try:
            ...
            traindata = renormalize(
                traindata, scenario["normalization_dict"], verbose=0
            )
            generator = AutoEncoderDataGenerator(
                traindata,
                scenario["batch_size"],
            ...
            profile_html = plot_autoencoder_profiles(
                model, scenario, generator, shots, times
            )
            control_html = plot_autoencoder_control_encoding(
                model,
                scenario,
                generator,
                shots,
                times,
                filename="control_encoding.png",
            )
            AB_html = plot_autoencoder_AB(model, scenario, filename="AB.png")
            spectrum_html = plot_autoencoder_spectrum(
                model, scenario, filename="spectrum.png"
            )
            train_html = plot_autoencoder_training(
                model, scenario, filename="training.png"
            )
...
def scenario_to_html(scenario):
    ...
    foo = {
        ...
        if k
        not in ["history", "normalization_dict", "history_params", "mse_weight_vector"]
    }
...
def get_submodels(model):
    ...
    state_decoder = Model(
        model.get_layer("state_decoder_time_dist").layer.layers[0].input,
        model.get_layer("state_decoder_time_dist").layer.layers[-2].get_output_at(1),
        name="state_decoder",
    )
    control_decoder = Model(
        model.get_layer("ctrl_decoder_time_dist").layer.layers[0].input,
...
def plot_autoencoder_training(model, scenario, filename=None, **kwargs):
    ...
    axes[0, 1].semilogy(
        scenario["history"]["x_residual_mean_squared_error"], label="train"
    )
    axes[0, 1].semilogy(
        scenario["history"]["val_x_residual_mean_squared_error"], label="val"
    )
    axes[0, 1].set_title("X residual MSE")
    axes[0, 1].legend()

    axes[1, 0].semilogy(
        scenario["history"]["u_residual_mean_squared_error"], label="train"
    )
    axes[1, 0].semilogy(
        scenario["history"]["val_u_residual_mean_squared_error"], label="val"
    )
    axes[1, 0].set_title("U residual MSE")
    axes[1, 0].legend()

    axes[1, 1].semilogy(
        scenario["history"]["linear_system_residual_mean_squared_error"], label="train"
    )
    axes[1, 1].semilogy(
        scenario["history"]["val_linear_system_residual_mean_squared_error"],
        label="val",
    )
...
def get_autoencoder_predictions(
    ...
    **kwargs
    ...
    for sig in scenario["profile_names"] + scenario["scalar_names"]:
        ...
        x0["input_" + sig] = inputs["input_" + sig][:, 0, :].reshape(
            (1, 1, scenario["profile_length"])
        )
    control_inputs = {}
    ...
    for i, sig in enumerate(scenario["profile_names"]):
        state_predictions[sig] = np.squeeze(
            np.array([x_decoded[j][i] for j in range(len(x_decoded))])
        )
        residuals[sig] = state_inputs[sig] - state_predictions[sig]
    return state_inputs, state_predictions, residuals


def plot_autoencoder_residuals(
    residuals, scenario, shot, timestep, filename=None, **kwargs
):
    ...
    fig.suptitle("Shot " + str(int(shot)) + "   Time " + str(int(timestep)), y=0.95)
    for j in range(nsteps):
        for i, sig in enumerate(scenario["profile_names"]):
            ax[j].plot(
                psi, residuals[sig][j].reshape((scenario["profile_length"],)), label=sig
            )
            ax[j].hlines(0, 0, 1)
            ax[j].legend()
            ax[j].tick_params(reset=True)
            ax[j].title.set_text(
                "State Residuals t+" + str(int(j * scenario["dt"] * 1000))
            )
    fig.tight_layout()
...
def plot_autoencoder_predictions_timestep(
    state_inputs, state_predictions, scenario, shot, timestep, filename=None, **kwargs
):
    ...
    baseline = {
        k: v[0].reshape((scenario["profile_length"],)) for k, v in state_inputs.items()
    }
    true = {
        k: v[-1].reshape((scenario["profile_length"],)) for k, v in state_inputs.items()
    }
    pred = {
        k: v[-1].reshape((scenario["profile_length"],))
        for k, v in state_predictions.items()
    }
    ...
    fig.suptitle(
        ...
        + str(int(timestep))
        + "   Prediction Window "
        + str(int(scenario["lookahead"] * scenario["dt"] * 1000)),
        y=0.95,
    )
    for i, sig in enumerate(scenario["profile_names"]):
...
def plot_autoencoder_profiles(model, scenario, generator, shots, times):
    ...
    for i, (shot, time) in enumerate(zip(actual["shots"], actual["times"])):
        ...
        newhtml = plot_autoencoder_predictions_timestep(
            state_inputs, state_predictions, scenario, shot, time, filename
        )
        html += newhtml + "\n"
...
def plot_autoencoder_control_encoding(
    model, scenario, generator, shots, times, filename=None, **kwargs
):
    ...
    fig.suptitle("Control Residuals", y=0.95)
    ...
    for j, (shot, time) in enumerate(zip(shots, times)):
        ...
        residuals = {
            sig: np.squeeze(inp["input_" + sig] - np.array(residuals)[:, i])
        ...
        for i, sig in enumerate(scenario["actuator_names"]):
            ...
            ax[np.unravel_index(j, (nrows, ncols))].title.set_text(
                "Shot " + str(int(shot)) + "   Time " + str(int(time))
            )
...
def plot_conv_training(model, scenario, filename=None, **kwargs):
    ...
    for i, targ in enumerate(targets):
        ...
        if "target_" + targ + "_mean_squared_error" in scenario["history"].keys():
            axes[idx].semilogy(
                scenario["history"]["target_" + targ + "_mean_squared_error"],
                label="train",
            )
            axes[idx].semilogy(
                scenario["history"]["val_target_" + targ + "_mean_squared_error"],
                label="val",
            )
            ...
            axes[idx].semilogy(
                scenario["history"]["target_" + targ + "_loss"], label="train"
            )
            axes[idx].semilogy(
                scenario["history"]["val_target_" + targ + "_loss"], label="val"
            )
...
def plot_conv_profiles(model, scenario, generator, shots, times):
    ...
    for i, (shot, time) in enumerate(zip(actual["shots"], actual["times"])):
        ...
        newhtml = plot_conv_profiles_timestep(
            model, scenario, inp, targ, shot, time, filename
        )
        html += newhtml + "\n"
    return html


def plot_conv_profiles_timestep(
    model, scenario, inputs, targets, shot, timestep, filename=None, **kwargs
):
    ...
    perturbed_input_names = [
        "input_future_" + sig for sig in scenario["actuator_names"]
    ]
    ...
    if scenario["predict_deltas"]:
        baseline = {
            sig: inputs["input_" + sig].reshape((scenario["profile_length"],))
            for sig in scenario["target_profile_names"]
        }
        true = {
            sig: targets["target_" + sig].reshape((scenario["profile_length"],))
            + baseline[sig]
            for sig in scenario["target_profile_names"]
        }
        pred = {
            sig: predictions[i].reshape((scenario["profile_length"],)) + baseline[sig]
            for i, sig in enumerate(scenario["target_profile_names"])
        }
        pred_pert = {
            sig: predictions_perturbed[i].reshape((scenario["profile_length"],))
            + baseline[sig]
            for i, sig in enumerate(scenario["target_profile_names"])
        }
    else:
        baseline = {
            sig: np.zeros(scenario["profile_length"])
            for sig in scenario["target_profile_names"]
        }
        true = {
            sig: targets["target_" + sig].reshape((scenario["profile_length"],))
        ...
        pred = {
            sig: predictions[i].reshape((scenario["profile_length"],))
            for i, sig in enumerate(scenario["target_profile_names"])
        }
        pred_pert = {
            sig: predictions_perturbed[i].reshape((scenario["profile_length"],))
    ...
    fig, ax = plt.subplots(nrows=nrows, ncols=ncols, figsize=figsize)
    fig.suptitle("Shot " + str(int(shot)) + "   Time " + str(int(timestep)), y=0.93)
    for i, sig in enumerate(scenario["target_profile_names"]):
        ax[np.unravel_index(2 * i, (nrows, ncols))].plot(
            psi,
            pred[sig] - baseline[sig],
            psi,
            true[sig] - baseline[sig],
            psi,
            pred_pert[sig] - baseline[sig],
        )
        ax[np.unravel_index(2 * i, (nrows, ncols))].title.set_text(sig + " (deltas)")
        ax[np.unravel_index(2 * i, (nrows, ncols))].hlines(0, 0, 1)
        ax[np.unravel_index(2 * i, (nrows, ncols))].legend(
            ["predicted delta", "true delta", "pred with perturbed input"]
        )

        ax[np.unravel_index(2 * i + 1, (nrows, ncols))].plot(
            psi, pred[sig], psi, true[sig], psi, pred_pert[sig], psi, baseline[sig]
        )
        ax[np.unravel_index(2 * i + 1, (nrows, ncols))].title.set_text(sig + " (full)")
        ax[np.unravel_index(2 * i + 1, (nrows, ncols))].legend(
            ["predicted", "true", "pred with perturbed input", "baseline"]
        )
    fig.tight_layout()
    fig.subplots_adjust(top=0.88)
    if filename:
