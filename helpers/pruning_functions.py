...
def remove_non_gas_feedback(data, verbose):
    ...
    if verbose:
        print("Removed {} samples".format(
            len(data['gas_feedback'])-sum(keep_sample)))
    if verbose:
...
def remove_non_beta_feedback(data, verbose):
    ...
    keep_sample = np.all(np.logical_and(
        data['beam_feedback_switch'] == 2, data['beam_feedback_power_target_quantity'] == 9), axis=1)
    if verbose:
        print("Removed {} samples".format(
...
def remove_nan(data, verbose):
    ...
    for sig in data.keys():
        if data[sig].ndim == 1:
            ...
            remove_inds += np.where(np.any(np.isnan(data[sig]), axis=ax))[
                0].tolist()
