...
def normalize_arr(data, method, uniform_over_profile=True):
    ...
    if method == "StandardScaler":
        ...
        param_dict.update({"method": method, "mean": mean, "std": std})
        ...
        param_dict.update({"method": method, "armin": armin, "armax": armax})
        return (data - armin) / np.maximum(
            (armax - armin), np.finfo(np.float32).eps
        ), param_dict
        ...
        param_dict.update({"method": method, "maxabs": maxabs})
        ...
        if uniform_over_profile or data.ndim < 3:
            ...
            y = yeo_johnson_transform(data.flatten(), lmbda).reshape(data.shape)
            mean = np.mean(y)
            std = np.std(y)
        else:
            y = np.zeros_like(data)
            lmbda = np.array(
                [yeo_johnson_optimize(data[:, :, i]) for i in range(data.shape[2])]
            )
            for i, l in enumerate(lmbda):
                y[:, :, i] = yeo_johnson_transform(data[:, :, i], l)
            mean = np.mean(y, axis=(0, 1))
            std = np.std(y, axis=(0, 1))
        param_dict.update({"method": method, "lambda": lmbda, "mean": mean, "std": std})
    ...
    if type(data) is dict:
        ...
        for key in tqdm(
            ...
            if key not in ["time", "shotnum"] and "error" not in key:
                data[key], p = normalize_arr(data[key], method, uniform_over_profile)
                param_dict[key] = p
        for key in data.keys():
            if "error" in key:
                data[key] = (
                    renormalize(
                        data[key] + denormalize_arr(data[key[6:]], param_dict[key[6:]]),
                        param_dict[key[6:]],
                    )
                    - data[key[6:]]
                )
    ...
    if type(data) is dict:
        ...
        for key in data.keys():
            if "error" in key:
                data[key] = (
                    denormalize_arr(
                        data[key] + renormalize(data[key[6:]], param_dict[key[6:]]),
                        param_dict[key[6:]],
                    )
                    - data[key[6:]]
                )
    ...
    if type(data) is dict:
        ...
        if param_dict["method"] == "StandardScaler":
            ...
            if param_dict["lambda"].size > 1:
                ...
                y = yeo_johnson_transform(y.flatten(), param_dict["lambda"]).reshape(
                    y.shape
                )
